...
async function count (dataSetId, jobId) {
    let job = await bigQuery.createQueryJob({
        query: `SELECT COUNT (*) AS count
                FROM ${PROJECT_ID}.${dataSetId}.${TABLES.SINGLE_BATCH}
                WHERE jobId = '${jobId}'`,
        location: 'US'
    });
...
module.exports = {
    /**
     * Counts the number of entries in the batching table for a specific jobId.
     *
     * @method count
     * @methodOf SingleBatch
    ...
    /**
     * Inserts a pick run in the pick_run_optimization Big Query table.
     *
     * @method insert
